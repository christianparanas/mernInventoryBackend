...
router.post("/newproduct", async (req, res) => {
  ...
  db.query(
    "INSERT INTO products (product_name, product_image, product_description, product_price, product_quantity ) VALUES (?, ?, ?, ?, ?)",
    [
      product.name,
      product.image,
      product.description,
...
router.post("/updateproduct", (req, res) => {
  ...
  db.query(
    "UPDATE products SET product_name = ?, product_description = ?, product_price = ?, product_quantity = ? WHERE product_id = ?",
...
router.delete("/deleteproduct/:id", (req, res) => {
  ...
  db.query("DELETE FROM products WHERE product_id = ?", id, (err, result) => {
    if (!err) {
      ...
      res.status(202).json({
        message:
          "This Item can't be deleted because it is referenced in the other tables.",
      });
    } else {
      res.status(400).json({
        message: "Server cannot be reached!",
